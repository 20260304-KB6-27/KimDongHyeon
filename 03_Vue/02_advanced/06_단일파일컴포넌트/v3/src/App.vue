<template>
  ...
  <!-- 게시글 목록 -->
  <PostList :posts="sortPosts" @delete="deletePost" @fix="fixedPost"></PostList>
</template>
...
const fixedPost = (fixPost) => {
  let index = posts.value.findIndex((item) => fixPost.no === item.no);
  posts.value[index].content = fixPost.content;
  posts.value[index].title = fixPost.title;
};
